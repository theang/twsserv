...
class DBSuite extends AnyFunSuite with DBJsonFormats with Logging {
  ...
  test("try creating ticker type record") {
    DB.createTables()
    val ticker = TickerTypeDB(0, "TEST", "EXC", "STK", BarSizes.MIN15, 2, Option.empty, Option.empty, Option.empty, Option.empty, Option.empty, Option.empty)
  ...
  test("try creating ticker data record for FUTURES") {
    DB.createTables()
    val ticker = TickerTypeDB(0, "TEST", "EXC", "FUT", BarSizes.MIN15, 2, Option.empty, Option.empty, Option.empty, Option.empty, Some("202306"), Option.empty)
    val tickerType: TickerLoadType = ticker
    TickerDataRepo.createTableIfNotExists(tickerType)
    val tickerDataTable = TickerDataTable.getQuery(tickerType)
    val clazz = new TickerDataTableGen(tickerType)
  ...
  test("try creating ticker tick records") {
    DB.createTables()
    val ticker = TickerTypeDB(0, "TEST", "EXC", "STK", BarSizes.TICK, 2, Option.empty, Option.empty, Option.empty, Option.empty, Option.empty, Option.empty)
    val tickerType: TickerLoadType = ticker
  ...
  test("try creating ticker tick records for FUTURES") {
    DB.createTables()
    val ticker = TickerTypeDB(0, "TEST", "EXC", "FUT", BarSizes.TICK, 2, Option.empty, Option.empty, Option.empty, Option.empty, Some("202306"), Option.empty)
    val tickerType: TickerLoadType = ticker
    val expectedLast = Seq(TickerTickLast(0, 10, 1.0, 1.0, 1, "A", pastLimit = true, unreported = false))
    TickerTickRepo.writeLast(tickerType, expectedLast)
    val expectedBidAsk = Seq(TickerTickBidAsk(0, 10, 1.0, 1.0, 1.0, 1.0, bidPastLow = true, askPastHigh = false))
