...
import serv1.client._
import serv1.db.repo.impl._
import serv1.db.{ArchiveFinishedJobsActor, JobUpdateActor, TickerDataActor}
import serv1.job._
...
object RestServer extends RouteConcatenation {
  ...
  def apply(host: String, port: Int): Behavior[Message] = Behaviors.setup { ctx =>
    ...
    val dataClientThrottlingActorRef = ctx.spawn(DataClientThrottlingActor(TWSClient), "dataClientThrottlingActor")
    val twsThrottlingClient = new TWSThrottlingClient(dataClientThrottlingActorRef)
    val multiClient = new MultiClient(twsThrottlingClient)
    ...
    val jobUpdateActorRef = ctx.spawn(JobUpdateActor(JobRepo), "jobUpdateActor")
    val tickerJobService = new TickerJobService(multiClient, JobRepo, tickerDataActor, jobUpdateActorRef, ExchangeRepo)
